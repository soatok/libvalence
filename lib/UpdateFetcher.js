...
module.exports = class UpdateFetcher {
     ...
     * @returns {string}
     */
    async getRandomMirror() {
        if (!sodium) sodium = await SodiumPlus.auto();
        let index = await sodium.randombytes_uniform(this.mirrors.length);
    ...
    async fetchUpdate(downloadPath, mirror = null, verifier = null) {
        if (!mirror) {
            mirror = await this.getRandomMirror();
        }
    ...
    async fetchUpdateList(projectName = '', channel = '') {
        let mirror = await this.getRandomMirror();
