...
module.exports = class UpdateVerifier {
    ...
    async addPublicKey(key) {
        ...
        if (typeof key === 'string') {
            this.publicKeys.push(await kr.loadAsymmetricPublicKey(key));
            return this;
        }

        if (Buffer.isBuffer(key)) {
            this.publicKeys.push(await kr.loadAsymmetricPublicKey(key.toString()));
            return this;
        }
    ...
    async verify(publicKeyID, signature, filePath) {
        ...
        for (let i = 0; i < this.publicKeys.length; i++) {
            curr = Buffer.from(await Utility.getPublicKeyId(this.publicKeys[i]));
